...
import { IVP_ITEM, nextIvpProblem, type IvpProblem } from '../generators/ivp'
import { checkScoped, previewOf, type Verdict } from '../lib/check'
import { IVP_LADDER, ladderOf, stepLadder } from '../lib/ladder'
...
export function IVP({ progress, prefs, onPrefs, onAnswer, onRung }: IVPProps) {
  const learning = prefs.ivpView === 'rule'
  const ladder = ladderOf(IVP_LADDER, progress, {
    rung: progress.ivpRung,
    run: progress.ivpRun,
  })
  ...
  return (
    <section>
      <div className="scope-bar">
        <div className="mode-bar">
          ...
          <button
            ...
            onClick={() => onPrefs({ ...prefs, ivpView: 'rule' })}
          >
            Learn
          </button>
          <button
            className={!learning ? 'chip chip-active' : 'chip'}
            onClick={() => onPrefs({ ...prefs, ivpView: 'drill' })}
          >
            Drill
          </button>
          <span className="meta-note">
            {learning
              ? 'The whole method, from the equation to a function of t.'
              : 'An equation and its initial values in, a function of t out.'}
          </span>
        </div>
        {!learning ? (
          <div className="mode-bar">
            ...
            <button
              className={prefs.ivpGuided ? 'chip chip-active' : 'chip'}
              onClick={() => onPrefs({ ...prefs, ivpGuided: true })}
            ...
            <button
              className={!prefs.ivpGuided ? 'chip chip-active' : 'chip'}
              onClick={() => onPrefs({ ...prefs, ivpGuided: false })}
              title="Every shape, straight away"
            >
              Everything
            </button>
            <span style={{ width: 10 }} />
      ...
      {learning ? (
        <IvpRule onDrill={() => onPrefs({ ...prefs, ivpView: 'drill' })} />
      ) : (
        <IvpDrill
          key={String(prefs.ivpGuided)}
          guided={prefs.ivpGuided}
...
interface Dealt {
  ...
  mode: 'choose' | 'type'
  tier: Tier
  choices: IvpProblem['choices']
  correctIndex: number
}

function trim(problem: IvpProblem, count: number) {
...
function IvpDrill({
  ...
  const build = useCallback((): Dealt => {
    ...
    const problem = nextIvpProblem({
      rung: guided ? ladderRef.current.rung : 3,
      uncovered: uncoveredItems([IVP_ITEM], state.facets),
    })
  ...
  const settle = useCallback(
    (correct: boolean, detail: AttemptDetail = {}) => {
      onAnswer([IVP_ITEM], correct, detail)
      setRecent((r) => [...r.slice(-19), correct])
      if (guided) {
        const next = stepLadder(IVP_LADDER, ladderRef.current, correct)
  ...
  return (
    <>
      {guided ? <LadderStrip ladder={IVP_LADDER} state={ladderRef.current} /> : null}
      ...
      <div className="card problem-card">
        <div className="problem-meta">
          ...
          <span className="meta-note">
            {problem.order === 1 ? 'First order' : problem.order === 2 ? 'Second order' : 'Third order'}
          </span>
        ...
        {mode === 'choose' ? (
          ...
          <AnswerBox
            ...
            label={
              <>
                Your answer, as a function of <code>t</code>
              </>
            }
            placeholder="2e^(3t) - e^(-t)"
            preview={preview}
